...
class PyBird(object):
    ...
    def __init__(self, socket_file, hostname=None, user=None, password=None, config_file=None, bird_cmd=None):
        ...
        self.routes_field_re = re.compile(r'(\d+) imported,( (\d+) filtered,)? (\d+) exported')
...
0019 Nothing to do
    ...
    def _parse_peer_detail(self, peer_detail_raw):
		...
		res = self.routes_field_re.findall(value)
		if res.__len__() > 0:
                    routes = res[0]
                    result['routes_imported'] = int(routes[0])
		    if routes.__len__() == 4:
                        result['routes_exported'] = int(routes[3])
		    else:
                        result['routes_exported'] = 0
